#!/usr/bin/env python3
"""Persistent, tool-neutral create-subagent workflow."""
...
from validate_agent import ALLOWED_MODEL_PROFILES, validate_agent_contract
...
def initialize_state(
    ...
    if not description.strip() or not goal.strip():
        raise ValueError("--description and --goal must not be empty")
    ...
    agents_root_path = _relative_path(agents_root, "--agents-root")
    full_name = _full_name(name, scope)
    agent_file = project_root / agents_root_path / f"{full_name}.md"

    state: dict[str, Any] = {
        "schema_version": 1,
        "workflow_id": str(uuid.uuid4()),
        "workflow_name": "create-subagent-workflow",
        "status": "waiting",
        ...
        "spec": {
            ...
            "goal": goal.strip(),
            "agents_root": agents_root_path.as_posix(),
            "model_profile": None,
            "skills": [],
            "isolation_note": None,
        ...
        "gate": {
            ...
            "message": "Complete capability decomposition, model profile, and isolation analysis.",
        },
    }
...
def _contract_template(state: dict[str, Any]) -> str:
    ...
    if spec["skills"]:
        skill_lines = "\n".join(
            f"- `{skill}`: `.agents/skills/{skill}/SKILL.md`" for skill in spec["skills"]
        )
    else:
        skill_lines = "- No deterministic Skill dependencies declared after explicit analysis."
    return f"""---
name: {spec['full_name']}
...
Host adapters must choose a currently supported concrete model and invocation format.
They must preserve this contract's isolation, dependencies, constraints, and return contract.
"""
...
def _dependency_guard(state: dict[str, Any]) -> tuple[int, str]:
    ...
    for skill in state["spec"]["skills"]:
        ...
        if not path.is_file():
            missing.append({"skill": skill, "path": str(path)})
    ...
    if not missing:
        ...
        return EXIT_SUCCESS, "Skill dependencies are present."
    ...
    error = {
        "at": _now(),
        "stage": "dependencies",
        "code": "SKILL_DEPENDENCIES_MISSING",
        "message": f"{len(missing)} declared Skill dependency(s) are missing",
        "retryable": retryable,
...
def _scaffold(state: dict[str, Any]) -> tuple[int, str]:
    ...
    if agent_file.exists():
        ...
        error = {
            ...
            "stage": "scaffold",
            "code": "TARGET_EXISTS",
            "message": f"Refusing to overwrite existing Agent Contract: {agent_file}",
            "retryable": False,
        }
        state["errors"].append(error)
    ...
    agent_file.write_text(_contract_template(state), encoding="utf-8")
    state["current_stage"] = "refine"
    state["status"] = "waiting"
    state["gate"] = {
        "type": "human_or_agent",
        "accepted_events": ["refined"],
        "message": "Refine the Agent Contract, then resume with event 'refined'.",
    }
    _record(state, "scaffolded", {"agent_contract": str(agent_file)})
    return EXIT_WAITING, state["gate"]["message"]


def _validate(state: dict[str, Any]) -> tuple[int, str]:
    state["attempts"]["validate"] += 1
    report = validate_agent_contract(
        Path(state["artifacts"]["agent_contract"]),
    ...
    error = {
        ...
        "message": f"Agent Contract validation failed with {len(report['issues'])} issue(s)",
        "retryable": retryable,
...
def resume_gate(
    ...
    if stage == "analyze" and event == "analyzed":
        if model_profile not in ALLOWED_MODEL_PROFILES:
            return EXIT_BLOCKED, state, "analyzed requires a supported --model-profile."
        if not isolation_note or not isolation_note.strip():
            return EXIT_BLOCKED, state, "analyzed requires --isolation-note."
        normalized_skills = []
        for skill in skills or []:
            if not SLUG_PATTERN.fullmatch(skill):
                return EXIT_BLOCKED, state, f"Invalid Skill slug: {skill}"
            if skill not in normalized_skills:
                normalized_skills.append(skill)
        state["spec"]["model_profile"] = model_profile
        state["spec"]["skills"] = normalized_skills
        state["spec"]["isolation_note"] = isolation_note.strip()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Persistent create-subagent workflow")
    ...
    init_parser.add_argument("--agents-root", default=".agents/agents")
    ...
    resume_parser.add_argument("--model-profile", choices=sorted(ALLOWED_MODEL_PROFILES))
    resume_parser.add_argument("--skill", action="append", default=[])
    resume_parser.add_argument("--isolation-note")
