...
> 2026-08-12 建（kg 跨仓迁移波，cg 催化剂定位职责③"可复用 skill 资产"承接）。
> 收 kg 清洗批 2 判决迁出的规程/清单/规格类资产——判断内核全部留 kg 判断层，
> 本 pack 存**可执行的操作面**，双宿主（claude/codex）经 SKILL.md description 原生发现。
...
| [sse-stream-parsing](skills/sse-stream-parsing/SKILL.md) | SSE 流式解析 carry buffer 规程 | 批 2 片 1"专业操作出 kg" |

维护约定：清单类内容（cross-host 验收状态等）会随宿主版本漂——更新时同步改"时点读数"日期；
蓝图祝福时点 commit 在 export.py 与 blueprint.md 附录两处，须同步改。
